<?php
	class Nilai extends CI_Controller
	{
		...
		function kelas()
		{
			...
			$siswa 			= "SELECT ts.nis, ts.nama 
							  FROM tbl_riwayat_kelas AS trk, tbl_siswa AS ts 
							  WHERE trk.nis = ts.nis AND trk.kd_kelas = '$kd_kelas' AND trk.id_tahun_akademik =". get_tahun_akademik('id_tahun_akademik') ." ";
		...
		function update_nilai_tugas()
		{
			...
			$parameter 	= array(
							...
							'nilai_uts' => $nilai_uts,
							'nilai_uas' => $nilai_uas,
							'nilai' =>  (0.3*$nilai_tugas) + (0.3*$nilai_uts) + (0.4*$nilai_uas)
						);
			...
			if ($check->num_rows() > 0) {
				...
				$this->db->update('tbl_nilai', array('nilai_tugas' => $nilai_tugas, 'nilai' => (0.3*$nilai_tugas) + (0.3*$nilai_uts) + (0.4*$nilai_uas)));
				echo "data diupdate";
			} else {
		...
		function update_nilai_uts()
		{
			...
			if ($check->num_rows() > 0) {
				...
				$this->db->where('nis', $nis);
				$this->db->where('id_jadwal', $idjadwal);
				$this->db->update('tbl_nilai', array('nilai_uts' => $nilai_uts, 'nilai' => (0.3*$nilai_tugas) + (0.3*$nilai_uts) + (0.4*$nilai_uas)));
				echo "data diupdate";
			} else {
				// Jika datanya tidak ada maka akan melakukan proses insert
				$this->db->insert('tbl_nilai', $parameter);
		...
		function update_nilai_uas()
		{
			...
			if ($check->num_rows() > 0) {
				...
				$this->db->where('nis', $nis);
				$this->db->where('id_jadwal', $idjadwal);
				$this->db->update('tbl_nilai', array('nilai_uas' => $nilai_uas, 'nilai' => (0.3*$nilai_tugas) + (0.3*$nilai_uts) + (0.4*$nilai_uas)));
				//$this->db->update('tbl_nilai', array('nilai' => (0.3*$nilai_tugas) + (0.3*$nilai_uts) + (0.4*$nilai_uas) ));
				echo "data diupdate";
			} else {
				// Jika datanya tidak ada maka akan melakukan proses insert
				$this->db->insert('tbl_nilai', $parameter);
...
/*
		function update_nilai()
		{
			...
			if ($check->num_rows() > 0) {
				...
				$this->db->where('nis', $nis);
				$this->db->where('id_jadwal', $idjadwal);
				$this->db->update('tbl_nilai', array('nilai' => (3*$nilai_tugas) + (3*$nilai_uts) + (4*$nilai_uas) ));
				echo "data diupdate";
			} else {
				// Jika datanya tidak ada maka akan melakukan proses insert
				$this->db->insert('tbl_nilai', $parameter);
