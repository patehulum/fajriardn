		...
		function index()
		{
			...
			elseif ($this->session->userdata('id_level_user') == 5) {
				...
				$sql = "SELECT tj.id_jadwal, tju.nama_jurusan, ttk.nama_tingkatan, tm.nama_mapel, tj.jam, 
						tr.nama_ruangan, tj.hari, tj.semester 
						FROM tbl_jadwal AS tj, tbl_jurusan AS tju, tbl_ruangan AS tr, tbl_mapel AS tm, tbl_tingkatan_kelas AS ttk
						WHERE tj.kd_jurusan = tju.kd_jurusan AND tj.kd_ruangan = tr.kd_ruangan AND tj.kd_mapel = tm.kd_mapel AND tj.kd_tingkatan = ttk.kd_tingkatan AND tj.kd_kelas = ?";
				$data['jadwal'] =$this->db->query($sql,array($kd_kelas));
				// load daftar ngajar guru
				$this->template->load('template', 'jadwal/jadwal_siswa', $data);
			}
		...
		function dataJadwal()
		{
			...
			foreach ($data_jadwal as $row) {
				echo "<tr>
						...
						<td  class='text-center'>".anchor('jadwal/delete/'.$row->id_jadwal, '<i class="fa fa-times fa fa-white"></i>', 'class="btn btn-xs btn-danger" data-placement="top" title="Delete"')."</td>
					 </tr>";
				$no++;
			}
