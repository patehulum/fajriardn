...
--
-- Host: 127.0.0.1
-- Generation Time: Dec 18, 2022 at 05:25 PM
...
INSERT INTO `tbl_service` (`id_service`, `no_plat`, `tanggal`, `kd_barang`, `qty`, `total`, `keterangan`) VALUES
(1, 'BD 9808 K', '2022-12-18', '000', 1, 90000, '- bermain'),
(2, 'BD 9808 K', '2022-12-18', '0', 3, 10000, NULL),
(3, 'BD 9808 K', '2022-12-18', '0', 0, 10000, NULL),
(4, 'OP 4547 PO', '2022-12-15', '000', 1, 20000, '- main mainin lagi'),
(5, 'OP 4547 PO', '2022-12-15', '5', 1, 10000, NULL);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_service`  AS SELECT `ts`.`id_service` AS `id_service`, `ts`.`no_plat` AS `no_plat`, `ts`.`tanggal` AS `tanggal`, `ts`.`kd_barang` AS `kd_barang`, `ts`.`qty` AS `qty`, `ts`.`total` AS `total`, `tc`.`nama_customer` AS `nama_customer` FROM (`tbl_service` `ts` join `tbl_customer` `tc` on(`tc`.`no_plat` = `ts`.`no_plat`))  ;
...
--
ALTER TABLE `tbl_service`
  MODIFY `id_service` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;
